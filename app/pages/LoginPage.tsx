...
import { MessageSquare, Send, User, Lock, Sparkles, AlertCircle, CheckCircle } from 'lucide-react';
import { useAuthStore } from '../store/useAuthStore';
import { Link, useNavigate } from 'react-router';

export default function ChatLogin() {
  const navigate = useNavigate();
  // Get login function from auth store outside of the handler
  const { login, isLogging } = useAuthStore();
  
  const [messages, setMessages] = useState([
    { text: "Welcome back to NebulaChat! Let's reconnect your cosmic journey.", sender: "bot" }
  ]);
  ...
  const getStepIcon = () => {
    switch(step) {
      case 0: return <User className="w-5 h-5 text-indigo-400" />;
      case 1: return <Lock className="w-5 h-5 text-indigo-400" />;
      default: return null;
    }
  };
  ...
  const getPlaceholder = () => {
    switch(step) {
      case 0: return "Your email address";
  ...
  const handleSubmit = (e) => {
    ...
    if (step === 0) {
      ...
      if (!emailRegex.test(input)) {
        setTimeout(() => {
          setMessages([...newMessages, { 
            text: "Hmm, that doesn't look like a valid email. Could you double-check?", 
            sender: 'bot',
            error: true
          }]);
        }, 600);
      ...
      setTimeout(() => {
        setMessages([...newMessages, { 
          text: "Got it! Now, please enter your password to unlock your cosmic conversations.", 
          sender: 'bot',
      ...
      setTimeout(() => {
        setMessages([...newMessages, { 
          text: "Great! Click 'Connect to Cosmos' to log in.", 
          sender: 'bot',
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Check if login was successful (the implementation may vary based on your auth store)
      // You might need to adjust this based on how your login function returns data
      if (response && response.success) {
        setTimeout(() => {
          setMessages([...messages, {
            text: `✨ Welcome back, ${response.user?.fullName || response.user?.name || 'Cosmic Traveler'}! Your journey continues.`,
            sender: 'bot',
            success: true 
          }]);
        }, 1000);
      ...
      let isUserNotRegistered = false;
      
      // Check for specific error types
      if (error.response) {
        // Check for user not found or unauthorized scenarios
        if (error.response.status === 404) {
          isUserNotRegistered = true;
          errorMessage = "No account found with these credentials. Would you like to create one?";
      ...
      setMessages(prev => [...prev, {
        ...
        sender: 'bot',
        error: true,
        isUserNotRegistered, // Flag to show signup button
  ...
  // Render action buttons with improved registration flow
  const renderActionButton = () => {
    // Check if the last message indicates a user not found
    const lastMessage = messages[messages.length - 1];
    const isUserNotRegistered = lastMessage?.isUserNotRegistered;
        ...
        <div className="flex flex-col space-y-3">
          ...
          <div className="text-center">
            <button 
              ...
              className={`${activeTheme.text} ${activeTheme.secondaryText} underline text-sm hover:opacity-80 transition-opacity`}
            >
              Try Again
            </button>
          </div>
  ...
  return (
    <div className={`flex flex-col lg:flex-row h-screen ${activeTheme.bg}`}>
      ...
      <div className="w-full lg:w-96 flex flex-col items-center justify-center relative overflow-hidden p-4 lg:p-8">
        ...
        <div className="relative z-10 mb-4 lg:mb-8 perspective">
          <div className="w-24 h-24 lg:w-32 lg:h-32 rounded-2xl bg-gradient-to-br bg-black flex items-center justify-center transform rotate-12 -translate-x-1 -translate-y-1 shadow-lg">
            <div className="w-20 h-20 lg:w-28 lg:h-28 rounded-xl bg-gradient-to-br from-black to-gray-800 flex items-center justify-center shadow-inner">
              <div className={`w-16 h-16 lg:w-24 lg:h-24 rounded-lg bg-gradient-to-br ${activeTheme.accent} flex items-center justify-center shadow-lg`}>
        ...
        <h1 className={`${activeTheme.text} text-3xl lg:text-4xl font-bold mb-2 lg:mb-3 relative z-10 tracking-tight`}>
          <span className="inline-block transform -rotate-3">Nebula</span>
          <span className="inline-block">Chat</span>
        </h1>
        <p className={`${activeTheme.secondaryText} text-center px-6 lg:px-10 relative z-10 font-light text-sm lg:text-base`}>
          Reconnect to your cosmic conversations
        </p>
        ...
        <div className="mt-4 lg:mt-8 flex justify-center gap-3 relative z-10">
          <button 
            ...
            className={`w-8 h-8 rounded-full bg-gradient-to-br from-violet-500 to-fuchsia-500 ${theme === 'cosmic' ? 'ring-2 ring-white' : ''}`}
          />
          <button 
            onClick={() => setTheme('aurora')} 
            className={`w-8 h-8 rounded-full bg-gradient-to-br from-emerald-400 to-teal-500 ${theme === 'aurora' ? 'ring-2 ring-white' : ''}`}
          />
          <button 
            onClick={() => setTheme('nebula')} 
            className={`w-8 h-8 rounded-full bg-gradient-to-br from-amber-500 to-rose-500 ${theme === 'nebula' ? 'ring-2 ring-white' : ''}`}
          />
        </div>
        ...
        <div className="mt-4 lg:mt-8 flex flex-col items-center gap-2 relative z-10">
          ...
          >
            <span className="inline-block group-hover:translate-y-[-1px] transition-transform duration-300 text-xs">
            Don&apos;t have an account?{" "}
              <span className="text-[#FF5E6C] underline">Create account</span>
            </span>
          </Link>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col mx-3 my-3 lg:mx-6 lg:my-6 rounded-2xl overflow-hidden backdrop-blur-xl border border-white border-opacity-10 shadow-2xl">
        <div className={`bg-gradient-to-r ${activeTheme.accent} p-4 lg:p-6 ${activeTheme.text}`}>
          <div className="flex items-center">
            ...
            <div className="ml-3 lg:ml-4">
              <h2 className="text-xl lg:text-2xl font-bold">Reconnect Your Space</h2>
              <p className={`${activeTheme.secondaryText} text-xs lg:text-sm`}>Your cosmic journey continues here</p>
            </div>
          </div>
        </div>
        
        {/* Chat Messages */}
        <div className={`flex-1 p-4 lg:p-6 overflow-y-auto bg-gradient-to-b ${activeTheme.chatBg}`}>
          <div className="space-y-4 lg:space-y-6">
            {messages.map((message, index) => (
              ...
              >
                {message.sender === 'bot' && (
                  <div className={`w-8 h-8 lg:w-10 lg:h-10 rounded-xl bg-gradient-to-br ${activeTheme.accent} flex items-center justify-center mr-2 lg:mr-3 shadow-lg`}>
                    {message.icon || <MessageSquare className="w-4 h-4 lg:w-5 lg:h-5 text-white" />}
                  </div>
                )}
        ...
        {/* Input Form */}
        <div className={`border-t border-white border-opacity-10 p-3 lg:p-5 ${activeTheme.chatBg} ${activeTheme.text}`}>
          {!isComplete ? (
            <form onSubmit={handleSubmit} className="flex items-center space-x-3 lg:space-x-4">
              <div className={`w-10 h-10 lg:w-12 lg:h-12 flex-shrink-0 rounded-xl ${activeTheme.inputBg} flex items-center justify-center border border-white border-opacity-10`}>
                {getStepIcon()}
              </div>
